...
Below are guides for Kubernetes and OpenTelemetry Collector approaches
...
Make copy of [otelcol-config.yaml](../config-examples/otelcol-config.yaml).  You can adjust this, but it has the config for scraping just metrics and sending them to the the Dynatrace OpenTelemetry Collector Config.
...
Review metrics in Dynatrace dashboard.  Just download the [NVIDIA-DCGM-dashboard.json](../config-examples/NVIDIA-DCGM-dashboard.json) dashbaord from this repo and upload this example into your environmment on the dashboard page.
